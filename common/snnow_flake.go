...
/*
* Reference: https://www.jianshu.com/p/a14c300e18f9
* 1                                               42           52             64
* +-----------------------------------------------+------------+---------------+
...
func (s *Snowflake) BatchGenerate(num int64) [] int64 {
	var result []int64
...
func NewSnowFlakeFromIP(ip string) (*Snowflake, error) {
	ipSlices := strings.Split(ip, ".")
	if len(ipSlices) != 4 {
		return nil, errors.New(fmt.Sprintf("Generate user id failed, bad ip format %s", ip))
	}
	workerId, err := strconv.ParseInt(strings.Join(ipSlices[2:], ""), 10, 64)
	if err != nil {
		return nil, err
	}
	return NewSnowFlake(workerId % WorkeridMax)
}
